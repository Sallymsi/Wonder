...
class SecurityController extends AbstractController
{
    ...
    public function signup(Request $request, UserAuthenticatorInterface $authenticator, FormLoginAuthenticator $formLogin, EntityManagerInterface $em, UserPasswordHasherInterface $passwordHasher, MailerInterface $mailer): Response
    {
        ...
        if ($userForm->isSubmitted() && $userForm->isValid()) {
            ...
            return $authenticator->authenticateUser($user, $formLogin, $request);
        }
    ...
    public function resetPassword(RateLimiterFactory $passwordRecoveryLimiter, Request $request, UserPasswordHasherInterface $userPasswordHasher, EntityManagerInterface $em, string $token, ResetPasswordRepository $resetPasswordRepository)
    {
    ...
    public function resetPasswordRequest(RateLimiterFactory $passwordRecoveryLimiter, MailerInterface $mailer, Request $request, EntityManagerInterface $em, UserRepository $userRepository, ResetPasswordRepository $resetPasswordRepository)
    {
        $limiter = $passwordRecoveryLimiter->create($request->getClientIp());
        if (false === $limiter->consume(1)->isAccepted()) {
            $this->addFlash('error', 'Vous devez attendre une heure pour refaire une tentative');
